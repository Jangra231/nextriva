...
export async function adminSeedDemoVenueEventsAction(formData: FormData) {
  const admin = await requireAdministratorAction("/admin?view=venues");
  if (text(formData.get("confirmation")) !== "MASTER") redirect("/admin?view=venues&error=Type+MASTER+to+confirm+the+demo+venue+event+load.");
  const outcome = await adminSeedDemoVenueEvents(admin.id);
  revalidatePath("/admin"); revalidatePath("/dashboard/manage-events/create-event"); revalidatePath("/venue");
  redirect(`/admin?view=venues&updated=${encodeURIComponent(`Demo venue event load complete: ${outcome.created} created, ${outcome.skipped} already-present skipped. ${outcome.venueCount} sample venues linked.`)}`);
}
